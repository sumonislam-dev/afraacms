@php
    ...
    $hasSeoErrors = $errors->hasAny(['seo_title', 'seo_canonical', 'seo_description', 'seo_image', 'seo_robots']);
    $hasSeoData = $seo && ($seo->meta_title || $seo->meta_description || $seo->canonical_url || $seo->meta_image || $seo->robots);
    $seoOpen = $hasSeoErrors || $hasSeoData;
@endphp
...
<div x-data="{ open: {{ $seoOpen ? 'true' : 'false' }} }" class="border-t border-gray-100 pt-6">
    <button type="button" x-on:click="open = ! open" class="flex w-full cursor-pointer items-center justify-between text-left">
        ...
        <span class="shrink-0 text-sm font-medium text-indigo-600 hover:text-indigo-900">
            <span x-show="! open" x-cloak>{{ __('Show SEO Fields') }}</span>
            <span x-show="open" x-cloak>{{ __('Hide SEO Fields') }}</span>
        </span>
    </button>

    <div x-show="open" x-cloak class="mt-4 space-y-4">
        <p class="text-sm text-gray-500">{{ __('Optional overrides. Anything left blank falls back to the site-wide defaults in Settings.') }}</p>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">{{ __('Blank unless this duplicates another URL.') }}</p>
