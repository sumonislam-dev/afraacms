<x-admin-layout :breadcrumbs="[['label' => __('Students'), 'url' => route('admin.students.index')], ['label' => __('Import')]]">
    <x-slot name="title">{{ __('Import Students') }}</x-slot>

    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-900">{{ __('Import Students') }}</h2>
    </x-slot>

    <x-admin.card>
        <div class="space-y-4">
            <p class="text-sm text-gray-600">
                {{ __('Upload an .xlsx or .csv file to register many students at once.') }}
                <a href="{{ route('admin.students.import.template') }}" class="font-medium text-indigo-600 hover:text-indigo-900">{{ __('Download the template') }}</a>
