...
>
    ...
    <div x-show="open" x-transition style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
        <button type="button" @click="open = false" aria-label="{{ __('Close') }}" class="absolute right-4 top-4 text-white hover:text-gray-300">
            <x-icon name="x-mark" class="h-8 w-8" />
        </button>

        <button type="button" @click="prev()" x-show="items.length > 1" aria-label="{{ __('Previous') }}" class="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:text-gray-300">
            <x-icon name="chevron-right" class="h-8 w-8 rotate-180" />
        </button>

        <button type="button" @click="next()" x-show="items.length > 1" aria-label="{{ __('Next') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:text-gray-300">
